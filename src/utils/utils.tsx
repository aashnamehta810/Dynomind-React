...
import defaultTranslation from '@app/locales/en/translation.json';
...
export const capitalize = (word: string): string => `${word ? `${word[0].toUpperCase()}${word.slice(1)}` : ''}`;
...
// find the http status severity and handle the case
export const checkHTTPStatus = (status: number, navigate: NavigateFunction): void => {
  const checkHTTPStatus = httpStatusCodes.find((item) => item.code === status);

  switch (checkHTTPStatus?.code) {
    case 400:
    case 401:
    case 500:
    case 502:
...
// regex to check only number
// eslint-disable-next-line
export const checkOnlyNumber: RegExp = /^[0-9]+$/;

export const contentDirection = (isRtl: boolean): DirectionType => {
  return isRtl ? 'rtl' : 'ltr';
};

export const getDefaultTranslationRow = (): Translation => ({
  name: 'english',
  isoCode: 'en',
  flagCode: 'gb',
  localHelper: 'en_US',
...
export const getAccessCode = (obj: PermissionData, value: string): number => {
  const checkAccess = Object.keys(obj).find((key: string) => key === value);
  return Number(checkAccess && obj[checkAccess]) || 0;
};

export const getRoutePermissionAccessCode = (
  obj: PermissionData,
  routeObj: PermissionData,
  pathname: string,
): number => {
  const matchedValue = Object.values(routeObj).find((value) => value === pathname);
  if (matchedValue) {
    const matchedKey = Object.keys(obj).find((key: string) => routeObj[key] === matchedValue);
    if (matchedKey) {
      return Number(obj[matchedKey]);
    }
  }
  return 0;
};
...
export const sortAscendingDecending = (astring: string, bstring: string): number => {
  return astring.localeCompare(bstring, undefined, { numeric: true });
};

export const getSchema = (val: ANY_OBJECT, keys: unknown[] = []): string | unknown[] => {
  return isObject(val) ? flatMap(val, (v, k) => getSchema(v, [...keys, k]) as string | unknown[]) : keys.join('.');
};
