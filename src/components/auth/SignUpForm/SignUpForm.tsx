...
import { checkHTTPStatus, randomString } from '@app/utils/utils';
...
export const SignUpForm: React.FC = () => {
  ...
  const defaultRole = process.env.REACT_APP_DEFAULT_ROLE;
  ...
  const handleSubmit = (values: SignUpFormData) => {
    ...
    if (!values.termOfUse) {
      notificationController.error({
        message: t('signup.acceptTermsAndPolicy'),
      });
      setLoading(false);
    } else {
      const payload = {
        ...
        email: values.email,
        password: isLoginTypeOTP ? randomString(8) : values.password,
        role: defaultRole,
